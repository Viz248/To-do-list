...
from models import Task, TaskCreate, TaskRead, UpdateTask#importing from models just to avoid clutter in 1 single file
...
@tasks_router.get("/")   #The "/" stuff is after the http://127.0.0.1:8000 so it would be http://127.0.0.1:8000/ in the end
def read_root():
...
@tasks_router.get("/hello/{name}")    #name here is gonna be dynamic, u can put anything
...
@tasks_router.get("/tasks", response_model=List[TaskRead])  #READ    response_model VALIDATES what the API returns to the user
...
@tasks_router.get("/tasks/{task_id}")    #Filtering by task id.
...
@tasks_router.put("/tasks/{task_id}/done", response_model=Task)    #UPDATE Marking task as done
...
@tasks_router.put("/tasks/{task_id}/title", response_model=Task)    #UPDATE Editing task
...
@tasks_router.patch("/tasks/{task_id}", response_model=Task)    #PARTIAL UPDATE Editing task and/or status
...
@tasks_router.delete("/tasks/{task_id}") #DELETE
...
@tasks_router.get("/search", response_model=List[TaskRead])  #Searching for task 
